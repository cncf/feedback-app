...
- [What actually happened to the 2024 feedback loop proposal?](https://github.com/cncf/feedback-app/issues/2) — parked, not dead and not duplicated: reached the End User TAB as `area/feedback-loops`, parked Dec 2025, missed KubeCon EU 2026; nothing of the design shipped. Revival is justified; coordination with the TAB is not optional.

## Not yet specified

- **Where the feedback hub lives.** The 2024 design needs a dedicated org with one repo per project. `cncf/feedback-app` is the app, not the hub. Whether the hub is a new org, a repo in `cncf`, or something smaller for the prototype is now an open question again — it was ruled out while the work was namespace-scoped.
- **Cross-org installation.** Back in scope now that this is CNCF-hosted: a real deployment installs into project orgs the driver does not control, which drags in App permission scope, approval flow, and the foundation's own policy.
...
- **Config file schema.** The 2024 doc sketches `Sources`, `FeedbackLabel`, `Labels`, `Categories`, `Managed`, `AllowedAuthors`. Whether the prototype needs a config file at all sharpens once the sync carrier is chosen.
