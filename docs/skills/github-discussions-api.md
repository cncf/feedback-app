...
hand-maintained, the reliable one is generated from a schema, and the
hand-maintained one is the more discoverable.
...
| `discussion` activity types | *Events that trigger workflows* lists 13, omitting `closed` and `reopened`. This is **staleness, not a narrower Actions subset** — both trigger workflows fine, proven by probe | [`/webhooks/webhook-events-and-payloads`](https://docs.github.com/en/webhooks/webhook-events-and-payloads) — `autogenerated: webhooks`, 15 actions |

Rule: **schema-generated references beat hand-maintained pages.** Check the
frontmatter — `autogenerated:` marks a page that tracks the platform
automatically. A hand-maintained list that disagrees with it is a docs bug until
proven otherwise, and the way to prove it is a probe, not a closer reading.
...
- **Latency floor.** Workflow dispatch is slower than a webhook handler; a poll
  is slower still.
- **`deleted`, and anything the hand-maintained trigger list genuinely lacks.**
  Verify by probe (below) rather than by reading the table.

**`closed` and `reopened` are *not* on this list — verified by execution.** See
the next section; the docs say otherwise and the docs are wrong.
