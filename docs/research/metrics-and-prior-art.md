...
**⇒ The proposal's topology (project repo → central feedback org) rules out `GITHUB_TOKEN`. Cross-repo discussion creation needs a GitHub App installation token or a PAT, and since this project forbids PATs, the *identity* must be an App installation.** That is a statement about the **credential, not the runtime**: a plain Actions workflow can mint a cross-owner installation token via [`actions/create-github-app-token`](https://github.com/actions/create-github-app-token) (`owner:` plus optional `repositories:`). A **two-workflow** carrier is therefore viable with no hosting — outbound in the source repo on `on: issues`, inbound in the hub repo on `on: discussion` / `on: discussion_comment`, each minting its own token — because a workflow does receive its own repository's events. The residual gap is narrow: the Actions trigger list is the 13-action subset, so `closed` and `reopened` never fire and those specific transitions need a poll or a webhook relay. Carrier choice belongs to [#6](https://github.com/cncf/feedback-app/issues/6) and should turn on that gap plus latency, not on the credential.
