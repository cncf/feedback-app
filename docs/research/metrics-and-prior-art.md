...
**⇒ The proposal's topology (project repo → central feedback org) rules out `GITHUB_TOKEN`. Cross-repo discussion creation needs a GitHub App installation token or a PAT, and since this project forbids PATs, the *identity* must be an App installation.** That is a statement about the **credential, not the runtime**: a workflow can mint a cross-owner installation token via [`actions/create-github-app-token`](https://github.com/actions/create-github-app-token) (`owner:` plus optional `repositories:`), and an Actions carrier receives every discussion activity type it needs — `closed` and `reopened` included, verified by probe on 2026-09-16 despite the Actions docs table omitting them. The real constraint on carrier choice is **private-key blast radius**: `actions/create-github-app-token` needs the App private key as a secret wherever it runs, and that key authenticates the App across *all* its installations, so a workflow in each of many third-party source orgs is not an acceptable shape. Carrier choice belongs to [#6](https://github.com/cncf/feedback-app/issues/6) and should turn on key custody and latency.
