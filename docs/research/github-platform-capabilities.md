...
page is hand-maintained. This looks like a `github/docs` bug. **Not reported by
this session**, and no existing report was found — but treat that as a weak
negative: the only searches run were `gh search issues/prs --repo github/docs`
over open and closed items for `discussion closed reopened trigger` and
`events-that-trigger-workflows discussion`, which returned nothing. Someone may
have filed it under wording those queries miss. Check before filing.
