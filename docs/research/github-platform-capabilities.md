...
**Conclusion: the 13-type Actions table is stale, not a narrower supported
subset.** `on: discussion` triggers on `closed` and `reopened` today. The
webhooks page carries `autogenerated: webhooks` in its frontmatter and tracks the
schema; the Actions page is hand-maintained and was not updated when
`closeDiscussion` / `reopenDiscussion` shipped. This is a `github/docs` bug and
